...
function checksessionStorage() {
    const savedValue = sessionStorage.getItem('selectedImages');
    if (savedValue) {
        // 更新内容
        ...
        selects.forEach((select, index) => {
            const selectValue = sessionStorage.getItem(`select_${index}`);
            if (selectValue !== null) {
...
selects.forEach((currentSelect, index) => {
    currentSelect.addEventListener('change', function(e) {
        ...
        if(selectedValue) {
            // 保存选择到本地存储
            sessionStorage.setItem('selectedImages', selectedValue);
            ...
            selects.forEach((otherSelect, otherIndex) => {
                if(otherIndex !== index) {
                    ...
                    otherSelect.value = "";
                    // 保存其他select的空状态
                    sessionStorage.setItem(`select_${otherIndex}`, "");
                }
            });
            
            // 保存当前select的状态
            sessionStorage.setItem(`select_${index}`, selectedValue);
